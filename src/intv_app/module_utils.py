...
def get_available_interview_types(modules_dir=None):
    """List available interview/module types by scanning for _vars.json files."""
...
def load_variable_schema(module_key, modules_dir=None):
    """Load and validate variable schema for a given module key."""
    if modules_dir is None:
        modules_dir = os.path.join(os.path.dirname(__file__), '../modules')
...
def prompt_for_variable(var, default_val):
    """Prompt user for a variable, prefilled with default value."""
    try:
        import readline
        def prefill():
