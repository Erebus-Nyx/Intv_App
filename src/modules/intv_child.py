# doc_intv_child.py
"""
Python equivalent of Doc_Intv_Child.bas
Implements Intv_Child_Output logic as a function (stub for now, expand as needed).

All major logic blocks and variable sourcing are now handled by the main pipeline and config/YAML/DB system.
For troubleshooting, see inline comments in each function and refer to the README.md for the variable resolution order and config structure.
"""
...
def intv_child_output(lookup_id=None, output_path=None, intv_data=None):
    with open(
        os.path.join(os.path.dirname(__file__), "..", "..", "config", "intv_child_vars.json"), "r", encoding="utf-8"
    ) as f:
    ...
    resolved = resolve_variables(lookup_id, logic_tree, db_lookup, user_prompt, provided=intv_data)
    for k, v in resolved.items():
        set_llm_variable(str(lookup_id), k, v)
    def get_personal_reference(resolved, default="the child"):
        first = resolved.get("Name", "").strip()
        last = resolved.get("LastName", "").strip()
        if first and last:
            return f"{first} {last}"
        elif first:
            return first
            ...
            return f"Mr./Ms. {last}"
        return default
    person_ref = get_personal_reference(resolved, default="the child")
    Tmp = ""
